...
<p align="center"><strong>MCO 为你的主 Agent 配备一支 Agent 团队：默认同时调度 Claude、Codex、Gemini、OpenCode、Qwen，并可显式选择 Copilot、Hermes 和 Pi。</strong></p>
...
OpenClaw 读取 `mco -h`，学会 CLI 接口，自主编排整个多 Agent 工作流。你的本地电脑变成一个多 Agent 审查团队 — OpenClaw 是管理者，MCO 是调度器，Claude/Codex/Gemini/OpenCode/Qwen 是默认团队成员。Copilot、Hermes 和 Pi 安装后可以显式选择。
...
- **环境健康检查** — `mco doctor` 探测默认 provider 集合的二进制、版本和认证状态；也可以显式指定 provider
...
- **Copilot、Hermes 和 Pi 适配器** — 通过显式 `--providers copilot,hermes,pi` 选择启用。
- **安全默认 provider 集合** — 默认仍为已验证的五个 provider：`claude,codex,gemini,opencode,qwen`。
...
| 通用执行 | `mco run --prompt "..."` | 多 provider 并行执行，返回最终文本 |
| 结构化审查 | `mco review --prompt "..."` | findings 合并、共识、决策输出 |
| 执行前预览 | `mco review --dry-run --json` | 解析 provider、policy、risk、命令模板，不实际运行 |
...
|----------|-----|------|--------------|
| GitHub Copilot CLI | `copilot` | 显式选择 | 非交互 Copilot CLI 运行，使用 `--allow-all-tools --no-ask-user`；不在默认集合中 |
...
| 执行前预览 | `mco review --dry-run --json` | 不运行 Agent，仅解析 provider、policy、risk 和命令 |
...
| 文件分工审查 | `mco review --divide files` | 按文件大小优先、均衡分配到不同 provider |
| 维度分工审查 | `mco review --divide dimensions` | 每个 provider 聚焦一个审查维度 |
| 实时 JSONL 流 | `mco review --stream jsonl` | provider 执行过程中输出 JSONL 事件流 |
| 实时终端流 | `mco review --stream live` | 交互式终端中的实时人类可读进度视图 |
| 查看 Agent 注册表 | `mco agent list` | 列出内置 + 自定义 Agent |
| 检查单个 Agent | `mco agent check my-ollama` | 验证某个自定义 Agent 或 Ollama 包装器 |
| 固定 provider 模型 | `mco run --provider-models-json '{"codex":"gpt-5.4"}'` | 覆盖所选 provider 的 CLI 默认模型 |
| 上下文策略 | `mco run --provider-context-json '{"pi":{"skills":"disabled","context_files":false}}'` | 按 provider 控制 skills / context files / plugins |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--providers` | `claude,codex,gemini,opencode,qwen` | 逗号分隔 provider 列表 |
...
| `--agent` | 未设置 | 临时自定义 ACP Agent：`--agent NAME "command"`，可与 `shim` 或 `acp` transport 一起使用 |
